...
public class PositionMap extends AbstractPositionReadWriteMap {
    ...
    private class MyDataIterator implements PositionDataIterator {
        ...
        public PositionData getNext() {
            ...
            synchronized (PositionMap.this) {
                ...
                do {
                    index++;
                } while (index < m_map.length && m_map[index] == null);
    ...
    private class MyGameModelIterator implements GameModelIterator {
        ...
        MyGameModelIterator(Iterator iterator) {
            m_iterator = iterator;
        }
        ...
        public GameModel nextGameModel() {
            return (GameModel) m_iterator.next();
        }
    ...
    static abstract class BasePositionData extends ChAbstractPositionData {
        ...
        protected short[] combineMovesPlayed(short[] moves1, short[] moves2) {
            if (moves1 == null) {
                short[] m = new short[moves2.length];
                System.arraycopy(moves2, 0, m, 0, moves2.length);
                return m;
            }
            ;
            if (moves2 == null) {
                short[] m = new short[moves1.length];
                System.arraycopy(moves1, 0, m, 0, moves1.length);
                return m;
            }
            ;
    ...
    public static class PositionDataGameShort extends AbstractPositionDataGame {
        ...
        public GameModel getGameModel() {
            return m_map.getGameModel(m_gameIndex);
        }

        protected short getNextMove() {
            return m_nextMove;
        }
    }
...
//            return res;
        ...
        public int getWhiteWins() {
            return (int) ((m_data >> SHIFT[0]) & MASK[0]);
        }

        public int getDraws() {
            return (int) ((m_data >> SHIFT[1]) & MASK[1]);
        }

        public int getBlackWins() {
            return (int) ((m_data >> SHIFT[2]) & MASK[2]);
        }

        public int getWhiteEloAverage() {
            return (getWhiteEloGames() == 0 ? 0 : (int) ((m_data >> SHIFT[3]) & MASK[3])) + MIN_ELO;
        }

        public int getWhiteEloGames() {
            return (int) ((m_data >> SHIFT[4]) & MASK[4]);
        }

        public long getWhiteElos() {
            return getWhiteEloGames() * getWhiteEloAverage();
        }

        public int getBlackEloAverage() {
            return (getBlackEloGames() == 0 ? 0 : (int) ((m_data >> SHIFT[5]) & MASK[5])) + MIN_ELO;
        }

        public int getBlackEloGames() {
            return (int) ((m_data >> SHIFT[6]) & MASK[6]);
        }

        public long getBlackElos() {
            return getBlackEloGames() * getBlackEloAverage();
        }

        public int getFirstOccurrence() {
            return (int) ((m_data >> SHIFT[7]) & MASK[7]) + MIN_YEAR;
        }

        //================================================================================

        private void incWhiteWins(long val) {
            m_data += (val << SHIFT[0]);
        }

        private void incDraws(long val) {
            m_data += (val << SHIFT[1]);
        }

        private void incBlackWins(long val) {
            m_data += (val << SHIFT[2]);
        }
...
//        m_gameScorer = new ChDefaultGameScorer();
    ...
    protected GameScorer getGameScorer() {
        return m_gameScorer;
    }
